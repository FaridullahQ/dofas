...
class McitReasonActionMixin(models.AbstractModel):
    ...
    `mcit.approval.mixin._transition`'s `comment`, or posting it to the
    ...
    instead of the raw action is all that's needed. `mcit.approval.mixin`
    already inherits this, so most models get it for free.

    A method name that doesn't exist on the target model is only ever an
    error path (mcit.reason.wizard.action_confirm raises a clear UserError)
    ...
    _name = "mcit.reason.action.mixin"
