...
export class ArcsDashboard extends Component {
    setup() {
        ...
        onWillStart(async () => {
            this.state.options = await this.orm.call("arcs.dashboard", "get_filter_options", []);
            await this.load();
        });
    }
    ...
    async load() {
        ...
        this.state.data = await this.orm.call("arcs.dashboard", "get_dashboard_data", [this.state.filters]);
    ...
    openExpenses() {
        ...
        this._openList("Expenses", "arcs.expense", d);
    }
    openCommitments() {
        const f = this.state.filters;
        const d = [["state", "=", "confirmed"]];
        if (f.grant_id) { d.push(["grant_id", "=", parseInt(f.grant_id, 10)]); }
        this._openList("Commitments", "arcs.commitment", d);
    }
    openFundReceipts() {
        const f = this.state.filters;
        ...
        if (f.grant_id) { d.push(["grant_id", "=", parseInt(f.grant_id, 10)]); }
        if (f.donor_id) { d.push(["grant_id.donor_id", "=", parseInt(f.donor_id, 10)]); }
        this._openList("Fund Receipts", "arcs.fund.receipt", d);
    }
    openAdvances() {
        const f = this.state.filters;
        ...
        if (f.zone_id) { d.push(["zone_id", "=", parseInt(f.zone_id, 10)]); }
        if (f.grant_id) { d.push(["grant_id", "=", parseInt(f.grant_id, 10)]); }
        this._openList("Advances", "arcs.advance", d);
    }
    openOverdueReports() {
        this._openList("Overdue Donor Reports", "arcs.donor.report", [["is_overdue", "=", true]]);
    }
    openPendingRequests() {
        const f = this.state.filters;
        ...
        if (f.grant_id) { d.push(["grant_id", "=", parseInt(f.grant_id, 10)]); }
        if (f.zone_id) { d.push(["zone_id", "=", parseInt(f.zone_id, 10)]); }
        this._openList("Pending Acquisitions", "arcs.spend.request", d);
    }
    openGrants() {
        const f = this.state.filters;
        const d = [];
        ...
        this._openList("Grants", "arcs.grant", d);
    }
    openGrant(id) {
        this.action.doAction({
            type: "ir.actions.act_window", res_model: "arcs.grant", res_id: id,
            views: [[false, "form"]], target: "current",
        });
    }
}
ArcsDashboard.template = "arcs_dashboard.Dashboard";

registry.category("actions").add("arcs_dashboard.main", ArcsDashboard);
