...
class ArcsExpense(models.Model):
    ...
    commitment_id = fields.Many2one("arcs.commitment", readonly=True, copy=False)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", _("New")) == _("New"):
                vals["name"] = self.env["ir.sequence"].next_by_code("arcs.expense") or _("New")
    ...
    def _get_journal(self):
        j = self.company_id.arcs_expense_journal_id
        if not j:
            j = self.env["account.journal"].search(
                [("type", "=", "general"), ("company_id", "=", self.company_id.id)], limit=1)
        if not j:
            raise UserError(_("Configure an Expense Journal in ARCS Settings."))
        return j
    ...
    def _create_actual_move(self):
        ...
        clearing = self.company_id.arcs_expense_clearing_account_id
        if not clearing:
            raise UserError(_("Configure an Expense Clearing Account in ARCS Settings."))
    ...
    def action_print_voucher(self):
        self.ensure_one()
        return self.env.ref("arcs_expense.action_report_expense_voucher").report_action(self)
