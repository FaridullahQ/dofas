...
class TestArcsFlow(TransactionCase):

    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.company = cls.env.company
        cls.plan = cls.env["account.analytic.plan"].create({"name": "ARCS Grants"})
        cls.company.arcs_default_analytic_plan_id = cls.plan
        cls.exp_acc = cls.env["account.account"].create(
            {"name": "Programme", "code": "ARCS600", "account_type": "expense"})
        cls.clearing = cls.env["account.account"].create(
            {"name": "Clearing", "code": "ARCS200", "account_type": "liability_current"})
        cls.journal = cls.env["account.journal"].create(
            {"name": "ARCS", "type": "general", "code": "ARCSX", "company_id": cls.company.id})
        cls.company.arcs_expense_journal_id = cls.journal
    ...
    def test_cash_availability(self):
        ...
        self.env["arcs.fund.receipt"].create({
    ...
    def test_audit_immutable(self):
        ...
        log = self.env["arcs.audit.log"].search(
            [("res_ref", "=", "arcs.grant,%s" % self.grant.id)], limit=1)
        self.assertTrue(log)
        with self.assertRaises(UserError):
            log.unlink()
