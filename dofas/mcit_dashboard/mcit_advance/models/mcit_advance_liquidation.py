...
class McitAdvanceLiquidation(models.Model):
    _name = "mcit.advance.liquidation"
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get("name") or vals.get("name") == _("New"):
                vals["name"] = (
                    self.env["ir.sequence"].next_by_code("mcit.advance.liquidation")
                    or _("New")
                )
    ...
    def _check_holder_ownership(self):
        ...
        """
        finance_mgr_group = self.env.ref("mcit_base.group_finance_manager", raise_if_not_found=False)
        sys_admin_group = self.env.ref("mcit_base.group_system_admin", raise_if_not_found=False)
    ...
    def action_post(self):
        for liq in self:
            ...
            if liq.advance_id.company_id.mcit_advance_book:
    ...
    def _create_liquidation_move(self):
        ...
        adv_account = c.mcit_advance_account_id
        clearing = c.mcit_advance_clearing_account_id
        if not journal or not adv_account or not clearing:
            raise UserError(_(
                ...
                "under Settings → MCIT Configuration before posting liquidations."
            ))
