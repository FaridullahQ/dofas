...
class McitDashboard(models.AbstractModel):
    """Filter-aware KPI/aggregate provider for the OWL portfolio dashboard.

    Snapshot figures (planned/committed/available) come from mcit.budget.line.
    Date-accurate actuals and all dimensional/time breakdowns are aggregated
    from POSTED mcit.expense records (the vouchers), so the dashboard can be
    ...
    _name = "mcit.dashboard"
    _description = "MCIT Grant Portfolio Dashboard (data provider)"
    ...
    def get_filter_options(self):
        ...
        programs = rows("mcit.program")
        projects = self.env["mcit.project"].search_read([], ["name", "program_id"]) \
            if "mcit.project" in self.env else []
        activities = self.env["mcit.activity"].search_read([], ["name", "project_id"]) \
            if "mcit.activity" in self.env else []
        grants = self.env["mcit.grant"].search_read(
            [("company_id", "=", company.id)], ["display_name", "donor_id"], order="date_start desc")
        donors = rows("mcit.donor")
        lines = self.env["mcit.budget.line"].search_read(
            [("grant_id.company_id", "=", company.id)], ["name", "grant_id"], order="name")
        zones = rows("mcit.zone") if "mcit.zone" in self.env else []
        return {
    ...
    def get_dashboard_data(self, filters=None):
        ...
        policy = company.mcit_currency_rate_policy or "transaction"
        today = fields.Date.context_today(self)
        F = self._parse(filters)

        Grant = self.env["mcit.grant"]
        Line = self.env["mcit.budget.line"]
        Expense = self.env["mcit.expense"]
        ...
        # received per grant
        received_g = {}
        if "mcit.fund.receipt" in self.env and scope_grants:
            for row in self.env["mcit.fund.receipt"].read_group(
        ...
        projmap = {}
        if "mcit.project" in self.env:
            for p in self.env["mcit.project"].search_read([], ["program_id"]):
        ...
        if "zone_id" in Expense._fields:
            zmap = {z["id"]: z["name"] for z in self.env["mcit.zone"].search_read([], ["name"])} \
                if "mcit.zone" in self.env else {}
            zc = {}
        ...
        if "mcit.advance" in self.env:
            Adv = self.env["mcit.advance"]
        ...
        if "mcit.spend.request" in self.env:
            rdom = [("state", "in", ("submitted", "committed"))]
            if F["grant"]:
                rdom += [("grant_id", "=", F["grant"])]
            if F["zone"]:
                rdom += [("zone_id", "=", F["zone"])]
            totals["pending_requests"] = self.env["mcit.spend.request"].search_count(rdom)
        overdue = 0
        if "mcit.donor.report" in self.env:
            odom = [("is_overdue", "=", True)]
            if "company_id" in self.env["mcit.donor.report"]._fields:
                odom += [("company_id", "=", company.id)]
            overdue = self.env["mcit.donor.report"].search_count(odom)
