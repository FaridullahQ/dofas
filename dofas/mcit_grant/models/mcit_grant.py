...
class McitGrant(models.Model):
    ...
    donor_id = fields.Many2one("mcit.donor", string="Donor", required=True, tracking=True,
                               domain="[('active','=',True)]")
    ...
    def _ensure_analytic_account(self):
        ...
        if self.analytic_account_id:
            return
        plan = self.company_id.mcit_default_analytic_plan_id
        if not plan:
            raise UserError(_("Configure a Default Grant Analytic Plan in MCIT Settings before creating grants."))
