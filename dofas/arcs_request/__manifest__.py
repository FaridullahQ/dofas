{
    ...
    "description": "Implements the 'Four Form' workflow: Programs drafts an acquisition, the budget "
                   ...
                   "'Requested By' is now an hr.employee and, once set, auto-fills Region, "
                   "Province, Department, Department Manager and Budget Holder from that "
                   "employee's own hr.department record - every field stays editable "
                   "afterwards. Once approved, Finance can disburse the approved amount to "
                   "that employee as a real cash advance (arcs_advance); the employee spends "
                   "it, their expense(s) are justified via a liquidation, and any difference "
                   "is settled with a real journal entry through a chosen bank/cash journal, "
                   "gated on a required slip attachment - paid back if they spent less, "
                   "reimbursed if they spent more from their own pocket. An approved "
                   "acquisition can spawn an expense that adopts the reserve instead of "
                   "re-reserving (or one expense per line if the reserve was split). If "
                   "Finance finds the budget line short, the request is routed to a "
                   "reportable 'Insufficient Funds' state with four recovery paths: choose a "
                   "different budget line, split the approved amount across several budget "
                   "lines, request an internal budget transfer, or request supplementary "
                   "donor funding.",
    ...
    "data": [
        ...
        "report/acquisition_request_summary_report.xml",
