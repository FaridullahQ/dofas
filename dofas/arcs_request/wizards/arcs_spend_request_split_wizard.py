...
class ArcsSpendRequestSplitWizard(models.TransientModel):
    ...
    Each portion becomes its own arcs.commitment (one per budget line used),
    ...
    _name = "arcs.spend.request.split.wizard"
    ...
    line_ids = fields.One2many(
        "arcs.spend.request.split.wizard.line", "wizard_id", string="Other Budget Lines")
...
class ArcsSpendRequestSplitWizardLine(models.TransientModel):
    _name = "arcs.spend.request.split.wizard.line"
    _description = "Split Reserve Line"

    wizard_id = fields.Many2one(
        "arcs.spend.request.split.wizard", required=True, ondelete="cascade")
