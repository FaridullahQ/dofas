...
class ArcsExpense(models.Model):
    ...
    request_id = fields.Many2one("arcs.spend.request", string="Acquisition",
    ...
    def _matching_request_commitment(self):
        ...
        see arcs.spend.request._action_create_split_expenses)."""
        self.ensure_one()
        if not self.request_id:
            return self.env["arcs.commitment"]
    ...
    def action_approve(self):
        ...
        rest = self - adopt
        if rest:
            super(ArcsExpense, rest).action_approve()
        return True
