...
class ArcsBudgetTransfer(models.Model):
    ...
    (both computed straight off approved arcs.budget.transfer records on
    arcs.budget.line - see _compute_amounts there), which is what actually
    ...
    arcs_request) can extend/link to."""

    _name = "arcs.budget.transfer"
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", _("New")) == _("New"):
                vals["name"] = self.env["ir.sequence"].next_by_code("arcs.budget.transfer") or _("New")
    ...
    def action_approve(self):
        ...
        for t in self:
            ...
            # amount (see arcs_budget_line.py._compute_amounts), so nothing
