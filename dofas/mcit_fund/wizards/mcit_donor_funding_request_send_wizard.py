...
class McitDonorFundingRequestSendWizard(models.TransientModel):
    ...
    sending. Unlike mcit.fund.receipt's acknowledgement email, an attachment
    ...
    _name = "mcit.donor.funding.request.send.wizard"
    _description = "Send Donor Funding Request Email"

    funding_request_id = fields.Many2one(
        "mcit.donor.funding.request", string="Funding Request", required=True,
    ...
    def default_get(self, fields_list):
        ...
        request = self.env["mcit.donor.funding.request"].browse(request_id)
        if request.exists():
    ...
    def _extra_body_html(self, request):
        """Extension point: downstream modules (e.g. mcit_request, when this
        ...
        duplicate the whole template - see mcit_request's override."""
        return ""
