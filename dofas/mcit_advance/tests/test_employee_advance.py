...
class TestMcitEmployeeAdvance(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.exp_acc = cls.env["account.account"].create(
            {"name": "Programme", "code": "MCITADV600", "account_type": "expense"})
        cls.adv_account = cls.env["account.account"].create(
            {"name": "Advances to Staff", "code": "MCITADV120", "account_type": "asset_current"})
        cls.company.write({
            "mcit_advance_book": True,
            "mcit_advance_journal_id": cls.env["account.journal"].search(
                [("type", "=", "general")], limit=1).id,
            "mcit_advance_account_id": cls.adv_account.id,
            "mcit_advance_cash_account_id": cls.env["account.journal"].search(
                [("type", "=", "cash")], limit=1).default_account_id.id,
            "mcit_advance_clearing_account_id": cls.exp_acc.id,
            "mcit_expense_clearing_account_id": cls.exp_acc.id,
        })
    ...
    def test_liquidation_blocked_above_advance_by_default(self):
        ...
        with self.assertRaises(ValidationError):
            self.env["mcit.advance.liquidation"].create({
