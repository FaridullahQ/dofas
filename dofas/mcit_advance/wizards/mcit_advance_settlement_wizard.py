...
class McitAdvanceSettlementWizard(models.TransientModel):
    ...
    _name = "mcit.advance.settlement.wizard"
    _description = "Settle Advance"

    advance_id = fields.Many2one(
        "mcit.advance", string="Advance", required=True, readonly=True, ondelete="cascade")
    ...
    def default_get(self, fields_list):
        ...
        advance = self.env["mcit.advance"].browse(advance_id)
        if advance.exists():
    ...
    def _create_settlement_move(self, direction):
        ...
        adv_account = company.mcit_advance_account_id
        if not adv_account:
            raise UserError(_(
                "Configure the Advance (Receivable) Account under Settings → MCIT "
