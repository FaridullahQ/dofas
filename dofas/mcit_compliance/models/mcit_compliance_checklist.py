...
class McitComplianceChecklist(models.Model):
    _name = "mcit.compliance.checklist"
    _description = "Compliance Checklist"
    _inherit = ["mail.thread"]

    name = fields.Char(required=True)
    donor_id = fields.Many2one("mcit.donor", string="Donor")
    line_ids = fields.One2many("mcit.compliance.checklist.line", "checklist_id", string="Items")
...
class McitComplianceChecklistLine(models.Model):
    _name = "mcit.compliance.checklist.line"
    _description = "Compliance Checklist Item"
    _order = "sequence, id"

    checklist_id = fields.Many2one("mcit.compliance.checklist", required=True, ondelete="cascade")
