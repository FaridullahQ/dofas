...
class ArcsFundReceiptSendWizard(models.TransientModel):
    ...
    _name = "arcs.fund.receipt.send.wizard"
    _description = "Send Donor Acknowledgement Email"

    fund_receipt_id = fields.Many2one(
        "arcs.fund.receipt", string="Fund Receipt", required=True, readonly=True,
        ondelete="cascade")
    ...
    def default_get(self, fields_list):
        ...
        receipt = self.env["arcs.fund.receipt"].browse(receipt_id)
        if receipt.exists():
