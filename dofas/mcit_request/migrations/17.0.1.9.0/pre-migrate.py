"""requested_by on mcit.spend.request changes from a Many2one to res.users to
...
def migrate(cr, version):
    cr.execute("""
        ...
        WHERE table_name = 'mcit_spend_request' AND column_name = 'requested_by'
    """)
    if cr.fetchone():
        cr.execute("""
            ALTER TABLE mcit_spend_request
