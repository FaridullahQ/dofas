...
def migrate(cr, version):
    cr.execute("""
        ...
        WHERE table_name = 'mcit_spend_request' AND column_name = 'requested_by_user_id_old'
    """)
    if not cr.fetchone():
        return
    cr.execute("""
        UPDATE mcit_spend_request AS r
        SET requested_by = e.id
        FROM hr_employee AS e
        WHERE e.user_id = r.requested_by_user_id_old
          AND r.requested_by IS NULL
    """)
    cr.execute("ALTER TABLE mcit_spend_request DROP COLUMN requested_by_user_id_old")
