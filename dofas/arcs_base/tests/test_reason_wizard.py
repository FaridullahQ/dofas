...
class TestArcsReasonWizard(TransactionCase):
    ...
    arcs_base itself defines no concrete workflow model. The full happy
    ...
    arcs_grant's reject-with-reason test)."""

    def test_empty_reason_blocked(self):
        partner = self.env["res.partner"].create({"name": "Test Partner"})
        wizard = self.env["arcs.reason.wizard"].create({
