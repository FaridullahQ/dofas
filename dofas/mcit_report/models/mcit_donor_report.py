...
class McitDonorReport(models.Model):
    _name = "mcit.donor.report"
    ...
    template_id = fields.Many2one("mcit.report.template", string="Template")
    ...
    def _onchange_pick_template(self):
        ...
        if not self.report_type:
            return
        Tmpl = self.env["mcit.report.template"]
    ...
    def _require_financial_narrative(self):
        for r in self:
            count = self.env["ir.attachment"].search_count(
                [("res_model", "=", "mcit.donor.report"), ("res_id", "=", r.id)])
            if count < 1:
                raise UserError(_(
    ...
    def report_fx_info(self):
        ...
        policy = ""
        if "mcit_currency_rate_policy" in comp._fields:
            policy = dict(comp._fields["mcit_currency_rate_policy"].selection).get(
                comp.mcit_currency_rate_policy, comp.mcit_currency_rate_policy or "")
    ...
    def report_narrative(self):
        self.ensure_one()
        out = []
        for d in self.env["mcit.department.report"].search([("grant_id", "=", self.grant_id.id)]):
    ...
    def report_compliance(self):
        ...
        if "mcit.compliance.checklist" in self.env:
            lists = self.env["mcit.compliance.checklist"].sudo().search(
        ...
        cnt = self.env["ir.attachment"].search_count(
            [("res_model", "=", "mcit.grant"), ("res_id", "=", self.grant_id.id)])
    ...
    def report_commitments(self):
        self.ensure_one()
        out = []
        if "mcit.commitment" in self.env:
            for c in self.env["mcit.commitment"].search([("grant_id", "=", self.grant_id.id)]):
