...
class McitAsset(models.Model):
    ...
    grant_id = fields.Many2one(
        "mcit.grant", string="Grant", required=True, ondelete="restrict",
    ...
    expense_id = fields.Many2one(
        "mcit.expense", string="Source Expense",
        domain="[('grant_id', '=', grant_id)]",
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get("code") or vals.get("code") == _("New"):
                seq = self.env["ir.sequence"].next_by_code("mcit.asset")
    ...
    def _check_can_move(self):
        ...
        for a in self:
            if a.locked_to_id and not self.env.user.has_group(
                    "mcit_asset.group_asset_manager"):
                raise AccessError(_(
    ...
    def _custody_location(self):
        ...
        if not loc:
            raise UserError(_(
                ...
                "Settings > MCIT Asset Register before moving assets."))
        return loc
    ...
    def _store_location(self):
        ...
        if not loc:
            raise UserError(_(
                ...
                "Settings > MCIT Asset Register before using 'Move to Store'."))
        return loc
    ...
    def _disposal_location(self):
        ...
        if not loc:
            raise UserError(_(
                ...
                "Settings > MCIT Asset Register before disposing assets."))
        return loc
    ...
    def _internal_picking_type(self):
        ...
        if not pt:
            raise UserError(_(
                ...
                "Asset Warehouse under Settings > MCIT Asset Register."))
        return pt
    ...
    def action_transfer(self):
        ...
        return self._open_wizard("mcit.asset.transfer.wizard",
                                 _("Transfer Asset"))

    def action_dispose(self):
        self.ensure_one()
        self._check_not_terminal()
        return self._open_wizard("mcit.asset.dispose.wizard",
                                 _("Dispose Asset"))

    def action_mark_lost(self):
        self.ensure_one()
        self._check_not_terminal()
        return self._open_wizard("mcit.asset.lost.wizard",
                                 _("Mark Asset Lost"))
