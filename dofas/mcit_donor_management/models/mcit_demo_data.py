...
DEMO_MODULE = "__mcit_demo__"

# Deletion order matters: children before parents, so FK/ondelete constraints
# never block cleanup. mcit.commitment has no cascade tie to its source
...
CLEANUP_ORDER = [
    ...
    "mcit.expense",
    "mcit.asset",
    "mcit.budget.transfer",
    "mcit.donor.funding.request",
    "mcit.spend.request",
    "mcit.commitment",
    "mcit.fund.receipt",
    "mcit.activity",
...
class McitDemoData(models.AbstractModel):
    ...
    tagged via ir.model.data (module=__mcit_demo__) so it can be found and
    unwound later without touching any real data the user has entered."""

    _name = "mcit.demo.data"
    ...
    def _ensure_prereqs(self, company):
        ...
        company before they can be approved (mcit.grant auto-creates an
    ...
    def _gen_geo(self, state):
        Zone = self.env["mcit.zone"]
        Dept = self.env["mcit.department"]
    ...
    def _gen_programs(self, state, today):
        Program = self.env["mcit.program"]
        Project = self.env["mcit.project"]
        Activity = self.env["mcit.activity"]
        active = state["grants"]["grant_active"]
    ...
    def _gen_acquisitions(self, state, today):
        SR = self.env["mcit.spend.request"]
        active = state["grants"]["grant_active"]
    ...
    def _gen_advances(self, state, today):
        ...
        Liquidation = self.env["mcit.advance.liquidation"]
